...
class CourseWithAttrs {
    ...
    finalize() {
        for (const code of this.codes) {
            const coursePattern = /(?<subject>[A-Z]{2,4})(?<number>\d{4})(?<term>[ABC])?/;
            const m = code.match(coursePattern);
            const ct = new CourseTaken(m.groups['subject'], 
            // NB: we strip the term suffix (e.g., the 'A' in BCHE4597A)
            m.groups['number'], '', null, 1.0, GradeType.ForCredit, 'A', 202330, Array.from(this.attributes).map(a => `ATTRIBUTE=${a}`).join(';'), true);
...
(function (CourseAttribute) {
    ...
    CourseAttribute["NonEngr"] = "EUNE";
    CourseAttribute["SasLanguage"] = "AULA";
    CourseAttribute["SasLastLanguage"] = "AULL";
    CourseAttribute["SasAdvancedLanguage"] = "AULA";
    CourseAttribute["RoboTechElective"] = "EMRT";
...
const WritingSeminarSsHTbs = new Map([
...
const NetsFullTechElectives = new Set([
    "BEPP 2800",
    "BIOL 4536",
    "CIS 2400",
    "CIS 2620",
...
const NetsLightTechElectives = new Set([
    "EAS 3060",
    "EAS 4030",
    "EAS 5060",
    "EAS 5450",
...
const CisMseNonCisElectivesRestrictions1 = new Set([
    "EAS 500",
    "EAS 5100",
    "EAS 5120",
    "EAS 5450",
...
const CisMseNonCisElectivesRestrictions2 = new Set([
    "FNCE 7370",
    "GAFL 5310",
    "GEOL 5700",
    "LARP 7430",
...
const CisMseNonCisElectives = new Set([
    "BE 5160",
    "BE 5210",
    "BE 5300",
    "PHYS 5585",
...
const RoboGeneralElectives = new Set([
    "CIS 5050",
    "CIS 5220",
    "CIS 5230",
    "CIS 5480",
...
function myAssertEquals(a, b, message = "") {
    if (a != b) {
...
class RequirementLabel extends DegreeRequirement {
    ...
    constructor(displayIndex, label) {
        super(displayIndex);
        this.label = label;
        this.cus = 0;
        this.remainingCUs = 0;
...
class RequirementNamedCourses extends DegreeRequirement {
    constructor(displayIndex, tag, courses) {
        super(displayIndex);
        this.tag = tag;
...
class RequirementAttributes extends DegreeRequirement {
    constructor(displayIndex, tag, attrs) {
        super(displayIndex);
        this.tag = tag;
        this.attrs = attrs;
    }
...
class RequirementNamedCoursesOrAttributes extends RequirementNamedCourses {
    constructor(displayIndex, tag, courses, attrs) {
        super(displayIndex, tag, courses);
        this.attrs = attrs;
    }
    satisfiedBy(courses) {
...
class RequirementNumbered extends DegreeRequirement {
    constructor(displayIndex, tag, subject, numberPredicate, courses = new Set([])) {
        super(displayIndex);
        this.tag = tag;
...
class RequirementNaturalScienceLab extends RequirementNamedCourses {
    ...
    satisfiedBy(courses) {
        ...
        let matched = courses.find((c) => this.courses.includes(c.code()) &&
            c.grading == GradeType.ForCredit &&
...
class RequireCis1100 extends DegreeRequirement {
    ...
    satisfiedBy(courses) {
        const found1100 = courses.find((c) => {
...
class RequirementTechElectiveEngineering extends DegreeRequirement {
    constructor(displayIndex, allowLinking) {
        super(displayIndex);
...
RequirementCsci40TechElective.techElectives = new Set();
...
class RequirementAdvancedEseElective extends DegreeRequirement {
    constructor(displayIndex, eseOnly) {
        super(displayIndex);
...
class RequirementEseProfessionalElective extends DegreeRequirement {
    constructor(displayIndex, froshLevelEngr, courses = []) {
        super(displayIndex);
...
class RequirementSpa extends DegreeRequirement {
    constructor(displayIndex, light) {
        super(displayIndex);
        this.light = light;
    }
    satisfiedBy(courses) {
...
class RequirementSsh extends RequirementAttributes {
    constructor(displayIndex, attrs) {
        super(displayIndex, exports.SsHTbsTag, attrs);
    }
    satisfiedBy(courses) {
        ...
        const mostPopularSubjectFirst = Array.from(counts.keys())
            ...
            .filter((s) => counts.get(s) >= 2);
        // prioritize writing+ethics courses and satisfying the Depth Requirement
        return courses.slice()
            .sort((a, b) => {
            if ((a.attributes.includes(CourseAttribute.Writing) || CsciEthicsCourses.includes(a.code())) //&&
            ) {
                return -1;
            }
...
function countBySubjectSshDepth(courses) {
...
function byHighestCUsFirst(a, b) {
    return b.courseUnitsRemaining - a.courseUnitsRemaining;
}
/** used when sorting CourseTaken[] */
function byLowestLevelFirst(a, b) {
...
class CourseTaken {
    constructor(subject, courseNumber, title, _3dName, cus, grading, letterGrade, term, rawAttributes, completed) {
        ...
        this.allAttributes.forEach((attr) => {
            Object.values(CourseAttribute).forEach((a) => {
                if (a == attr) {
...
exports.CourseTaken = CourseTaken;
CourseTaken.NextUuid = 0;
...
class CourseParser {
    ...
    postProcess(courses, degrees, autoDegree) {
        // determine 37 vs 40 CUs
        const cusPerTerm = new Map();
        ...
        equivalentCourses.forEach((forbidden) => {
            if (courses.some((c) => c.code() == forbidden[0]) &&
                courses.some((c) => c.code() == forbidden[1])) {
                const msg = `took ${forbidden[0]} & ${forbidden[1]}, uh-oh: disabling ${forbidden[0]}`;
                myLog(msg);
                console.log(msg);
                let c0 = courses.find((c) => c.code() == forbidden[0]);
        ...
        if (!courses.some((c) => c.code() == "MATH 1400")) {
            const math1400Retro = courses.find((c) => {
            ...
            if (math1400Retro != undefined) {
                ...
                myLog(msg);
                const math1400 = new CourseTaken("MATH", "1400", Math1400RetroTitle, "MATH 104", 1.0, GradeType.ForCredit, "TR", math1400Retro.term, "ATTRIBUTE=EUMA; ATTRIBUTE=EUMS;", true);
        ...
        const halfCuCourses = [];
        courses.forEach(c => {
            const nosplit = (["PHYS 0151", "PHYS 0171", "ESE 1120"].includes(c.code()) &&
...
class DegreeWorksClassHistoryParser extends CourseParser {
    ...
    async parse(text, degrees) {
        ...
        if (minorCourseText != null) {
            ...
            const minorNames = Array.from(new Set(allMinors.map((v, i, a) => v)));
        ...
        courseLines.forEach(line => {
            ...
            if (hits != null) {
                ...
                const c = this.parseOneCourse(subject, number, hits.groups["title"], hits.groups["cus"], hits.groups["grade"], term);
                if (c != null) {
    ...
    parseOneCourse(subject, number, title, cus, grade, term) {
        ...
        return new CourseTaken(subject, number, title, null, parseFloat(cus), gradeType, grade, term, "", !inProgress);
    }
}
class DegreeWorksDiagnosticsReportParser extends CourseParser {
    ...
    extractCourses(worksheetText) {
        ...
        while (numHits < 100) {
            ...
            let c = this.parseOneCourse(hits.groups["subject"], hits.groups["number"], hits.groups["restOfLine"], hits.groups["attributes"]);
            if (c != null) {
                let dupe = coursesTaken.some((a) => a.subject == c.subject &&
        ...
        if (hits != null) {
            ...
            hits.groups["details"].split("\n")
                ...
                line.substring("Applied:".length).split(")").forEach(c => {
                    ...
                    let course = coursesTaken.find((c) => c.code() == name || c._3dName == name);
                    if (course != undefined) {
    ...
    parseOneCourse(subject, courseNumber, courseInfo, rawAttrs) {
        ...
        if (_4d != "") {
            ...
            return new CourseTaken(_4dparts[0], _4dparts[1], title, code, creditUnits, gradingType, letterGrade, term, rawAttrs, !inProgress);
        }
        // student took or is taking 4d course
        return new CourseTaken(subject, courseNumber, title, null, creditUnits, gradingType, letterGrade, term, rawAttrs, !inProgress);
    }
    parseDegrees(worksheetText) {
        let d = new Degrees();
        // undergrad degrees
...
class DegreeWorksStudentDataReportParser extends CourseParser {
    ...
    extractCourses(worksheetText) {
        ...
        while (numHits < 100) {
            ...
            let c = this.parseOneCourse(hits.groups["subject"], hits.groups["number"], hits.groups["term"], hits.groups["restOfLine"]);
...
class Degrees {
    constructor() {
        ...
        this.undergradMajorCatalogYear = undefined;
    }
    toString() {
        let s = "";
...
function snapCourseIntoPlace(course, req) {
...
function runTestInputDWDiagnosticsReport() {
...
function runTestInputDWClassHistory() {
...
async function webMain() {
    ...
    if (autoDegrees) {
        ...
        degrees.undergrad = $("input[name='ugrad_degree']:checked").val();
        degrees.masters = $("input[name='masters_degree']:checked").val();
    ...
    const allCourses = coursesTaken.slice()
        .sort((a, b) => a.code().localeCompare(b.code()))
        .map((c) => `<div><small>${c.toString()}</small></div>`).join("");
...
Math+Natural Science = ${result.gpaMathNatSci.toFixed(2)}
    ...
    if (result.requirementOutcomes.some(ro => ro.ugrad)) {
        ...
        renderRequirementOutcomesWeb(result.requirementOutcomes.filter(ro => ro.ugrad), NodeColumn1Reqs, NodeColumn2Reqs);
        if (result.requirementOutcomes.some(ro => !ro.ugrad)) {
            $(NodeDoubleColumn).removeClass("col-xl-12")
                .addClass("col-xl-8");
            $(NodeColumn3Header).append(`<h3>${degrees.masters} Degree Requirements</h3>`);
            renderRequirementOutcomesWeb(result.requirementOutcomes.filter(ro => !ro.ugrad), NodeColumn3Reqs, undefined);
        }
    }
        ...
        renderRequirementOutcomesWeb(result.requirementOutcomes.filter(ro => !ro.ugrad), NodeColumn1Reqs, undefined);
    }
    ...
    $(".course").delay(100).draggable({
        ...
        start: function (e, ui) {
            ...
            const course = $(this).data(DraggableDataGetCourseTaken);
    ...
    const updateGlobalReqs = function () {
        ...
        const sshCourses = coursesTaken
            .filter(c => c.consumedBy != undefined && c.consumedBy.toString().startsWith(exports.SsHTbsTag));
        // console.log(`updateGlobal SSH courses: ${sshCourses.map(c => c.code())}`)
        const updateGlobalReq = function (req) {
    ...
    $(".droppable").delay(100).droppable({
        ...
        activate: function (event, ui) {
            const req = $(this).data(DroppableDataGetDegreeRequirement);
            const course = ui.draggable.data(DraggableDataGetCourseTaken);
        ...
        drop: function (event, ui) {
            const destReq = $(this).data(DroppableDataGetDegreeRequirement);
            const realCourse = ui.draggable.data(DraggableDataGetCourseTaken);
            ...
            if (destReq.coursesApplied.includes(realCourse)) {
                ...
                const originReq = ui.draggable.data(DraggableOriginalRequirement);
                const crossDegree = (ugradDegreeReqs.includes(originReq) && mastersDegreeReqs.includes(destReq)) ||
...
function renderRequirementOutcomesWeb(requirementOutcomes, column1Id, column2Id) {
    requirementOutcomes.forEach((ro, i, allReqs) => {
...
function countRemainingCUs(allReqs) {
...
function probationRisk(rr, allCourses, termsThisYear) {
...
class RunResult {
    constructor(requirementOutcomes, cusRemaining, unconsumedCourses, overallGpa, stemGpa, mathNatSciGpa) {
...
function run(csci37techElectiveList, degrees, coursesTaken) {
    ...
    switch (degrees.undergrad) {
        case "40cu CSCI":
            ugradDegreeRequirements = [
                ...
                new RequirementNamedCoursesOrAttributes(10, "Natural Science", ["LING 2500", "PSYC 1340", "PSYC 121"], [CourseAttribute.NatSci]),
        ...
        case "40cu CMPE":
            ugradDegreeRequirements = [
                ...
                new RequirementNamedCoursesOrAttributes(29, "Tech Elective", ["ESE 4000", "EAS 5450", "EAS 5950", "MGMT 2370", "OIDD 2360"], [CourseAttribute.MathNatSciEngr])
                    .withMinLevel(2000),
        ...
        case "40cu NETS":
            ugradDegreeRequirements = [
                ...
                new RequirementNamedCourses(30, exports.SsHTbsTag, ["ECON 2100"]).withPassFail(),
                new RequirementNamedCourses(31, exports.SsHTbsTag, ["ECON 4100", "ECON 4101", "ECON 6110"]).withPassFail(),
                ...
                new RequirementSsh(35, [CourseAttribute.TBS, CourseAttribute.Humanities, CourseAttribute.SocialScience]),
                new RequirementSsh(36, [CourseAttribute.TBS, CourseAttribute.Humanities, CourseAttribute.SocialScience]),
                new RequirementAttributes(24, "Tech Elective (Light)", [CourseAttribute.NetsLightTechElective, CourseAttribute.NetsFullTechElective]),
        ...
        case "40cu EE":
            ugradDegreeRequirements = [
                ...
                new RequirementNamedCourses(22, "ESE Lab", ["ESE 2900", "ESE 2910", "ESE 3190", "ESE 3360",
                ...
                new RequirementEseProfessionalElective(28, false, ["ESE 4000", "EAS 5450", "EAS 5950", "MGMT 2370", "OIDD 2360"]),
                new RequirementNamedCourses(30, exports.SsHTbsTag, ["EAS 2030"]),
                new RequirementSsh(31, [CourseAttribute.SocialScience]),
                new RequirementSsh(32, [CourseAttribute.Humanities]),
                new RequirementSsh(33, [CourseAttribute.Humanities]),
                new RequirementSsh(34, [CourseAttribute.SocialScience, CourseAttribute.Humanities]),
        ...
        case "40cu SSE":
            ugradDegreeRequirements = [
                ...
                new RequirementNamedCourses(23, "System Project Lab", ["ESE 2900", "ESE 2910", "ESE 3500", "ESE 4210", "ESE 5050", "BE 4700"]).withCUs(1.5),
                new RequirementNamedCourses(24, "Senior Design", SeniorDesign1stSem),
                new RequirementNamedCourses(25, "Senior Design", SeniorDesign2ndSem),
                new RequirementNamedCourses(26, "Tech Management Elective", ["ESE 4000", "EAS 5450", "EAS 5950", "MGMT 2370", "OIDD 2360"]),
        ...
        case "37cu CMPE":
            ugradDegreeRequirements = [
                ...
                new RequirementNamedCourses(24, "Senior Design", SeniorDesign1stSem),
                new RequirementNamedCourses(25, "Senior Design", SeniorDesign2ndSem),
                new RequirementAttributes(26, "Professional Elective", [CourseAttribute.Math, CourseAttribute.NatSci, CourseAttribute.MathNatSciEngr]).withMinLevel(2000),
                new RequirementNamedCoursesOrAttributes(27, "Professional Elective", ["ESE 4000", "EAS 5450", "EAS 5950", "MGMT 2370", "OIDD 2360"], [CourseAttribute.Math, CourseAttribute.NatSci, CourseAttribute.MathNatSciEngr]).withMinLevel(2000),
                new RequirementAttributes(28, "Professional Elective", [CourseAttribute.Math, CourseAttribute.NatSci, CourseAttribute.MathNatSciEngr]),
                new RequirementAttributes(29, "Professional Elective", [CourseAttribute.Math, CourseAttribute.NatSci, CourseAttribute.MathNatSciEngr]),
        ...
        case "37cu NETS":
            ugradDegreeRequirements = [
                ...
                new RequireCis1100(9),
                new RequirementAttributes(23, "Tech Elective (Light)", [CourseAttribute.NetsLightTechElective, CourseAttribute.NetsFullTechElective]),
        ...
        case "37cu BE":
            ugradDegreeRequirements = [
                ...
                new RequirementNumbered(13, "BE Elective (4000/5000-level)", "BE", function (x) { return x >= 4000 && x < 6000; }),
                new RequirementNumbered(14, "BE Elective (4000/5000-level)", "BE", function (x) { return x >= 4000 && x < 6000; }),
    ...
    if (ugradDegreeRequirements.length > 0) {
        const displayIndices = new Set(ugradDegreeRequirements.map(r => r.displayIndex));
    ...
    switch (degrees.masters) {
        case "CIS-MSE":
            ...
            mastersDegreeRequirements = [
                ...
                new RequirementNamedCourses(3, "Core non-ML", ["CIS 5050", "CIS 5480", "CIS 5530", "CIS 4550", "CIS 5550", "CIS 4710", "CIS 5710", "CIS 5020", "CIS 5110", "CIS 5000"]),
                new RequirementNamedCourses(4, "Core", ["CIS 5050", "CIS 5480", "CIS 5530", "CIS 4550", "CIS 5550", "CIS 4710", "CIS 5710", "CIS 5020", "CIS 5110", "CIS 5000", "CIS 4190", "CIS 5190", "CIS 5200", "CIS 4210", "CIS 5210"]),
                new RequirementNumbered(5, "CIS Elective [5000,7000]", "CIS", function (x) { return cisCrossListNumbers.includes(x) || (x >= 5000 && x <= 7000); }),
                new RequirementNumbered(6, "CIS Elective [5000,6999]", "CIS", function (x) { return cisCrossListNumbers.includes(x) || (x >= 5000 && x < 7000); }),
                new RequirementNumbered(7, "CIS Elective [5000,6999]", "CIS", function (x) { return cisCrossListNumbers.includes(x) || (x >= 5000 && x < 7000); }),
                new RequirementNumbered(8, "Elective + Restriction 1", "CIS", function (x) { return cisCrossListNumbers.includes(x) || (x >= 5000 && x < 8000); }, new Set([...CisMseNonCisElectives, ...CisMseNonCisElectivesRestrictions1])),
                new RequirementNumbered(9, "Elective + Restriction 2", "CIS", function (x) { return cisCrossListNumbers.includes(x) || (x >= 5000 && x < 8000); }, new Set([...CisMseNonCisElectives, ...CisMseNonCisElectivesRestrictions2])),
                new RequirementNumbered(10, "Elective", "CIS", function (x) { return cisCrossListNumbers.includes(x) || (x >= 5000 && x < 8000); }, new Set([...CisMseNonCisElectives])),
            ];
            break;
        case "CGGT":
            mastersDegreeRequirements = [
                ...
                new RequirementNamedCourses(5, "Math", ["CIS 4190", "CIS 5190", "CIS 5200", "CIS 4610", "CIS 5610", "CIS 5630", "CIS 5800", "CIS 5810", "ENM 5030"]),
                ...
                new RequirementNamedCourses(10, "Free Elective", ["DSGN 5009", "FNAR 5066", "DSGN 5004", "EAS 5460", "OIDD 6620"]
    ...
    if (mastersDegreeRequirements.length > 0) {
        const displayIndices = new Set(mastersDegreeRequirements.map(r => r.displayIndex));
    ...
    coursesTaken.sort((a, b) => a.courseNumberInt - b.courseNumberInt);
    ...
    return new RunResult(reqOutcomes.map((o) => {
